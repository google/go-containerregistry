...
// Package manifest provides an MCP tool for getting the manifest of an image.
package manifest
...
// NewTool creates a new manifest tool.
func NewTool() mcppkg.Tool {
	return mcppkg.NewTool("manifest",
		mcppkg.WithDescription("Get the manifest of an image"),
		mcppkg.WithString("image",
			mcppkg.Required(),
			mcppkg.Description("The image reference to get the manifest for"),
		),
		mcppkg.WithBoolean("pretty",
			mcppkg.Description("Pretty print the manifest JSON"),
			mcppkg.DefaultBool(true),
		),
	)
}

// Handle handles manifest tool requests.
func Handle(ctx context.Context, request mcppkg.CallToolRequest) (*mcppkg.CallToolResult, error) {
	...
	if err != nil {
		return mcppkg.NewToolResultError(fmt.Sprintf("Error getting manifest: %v", err)), nil
	}
	...
	if pretty {
		...
		if err := json.Unmarshal(manifest, &obj); err != nil {
			return mcppkg.NewToolResultError(fmt.Sprintf("Error parsing manifest: %v", err)), nil
		}
		result, err = json.MarshalIndent(obj, "", "  ")
		if err != nil {
			return mcppkg.NewToolResultError(fmt.Sprintf("Error formatting manifest: %v", err)), nil
		}
	} else {
		result = manifest
	}
